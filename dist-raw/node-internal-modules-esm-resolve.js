...
function moduleResolve(specifier, base, conditions) {
  if (specifier.includes('*')) {
    return resolveGlobImport(specifier, base, conditions)
  }
...
function createRandomFileName(length, alphabet = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789') {
  const chars = []
...
function createTextForGlobResolvedImportFile({ cwd, absPath, imports, specifier }) {
  const metaInformationChunks = [
    `// Generated by node-ts to resolve glob imports within its runtime\n\n`,
    `// importing in: ${normalizePathToPosix(path.relative(cwd, absPath))}\n`,
    `// importing glob pattern: ${specifier}`
  ]
  ...
  const generalChunks = [
    ...metaInformationChunks,
    '\n\n',
    ...exportChunks
  ]

  return generalChunks.join('')
}

function resolveGlobImport(specifier, base, conditions) {
  const cwd = process.cwd()

  const absPath = url.fileURLToPath(base)
  const absPathWithoutFile = path.dirname(absPath)

  const finalPath = path.join(absPathWithoutFile, specifier)

  const globFiles = fastGlob.globSync(
    normalizePathToPosix(finalPath)
  )

  const globFromNormalizedBase = fastGlob.globSync(
    specifier,
    { cwd: absPathWithoutFile }
  )

  const tempFolderName = '.ts_node_temp'

  const tempFolderPath = path.join(cwd, tempFolderName)

  fs.mkdirSync(tempFolderPath, { recursive: true })
  
  const tempPathId = createRandomFileName(8)

  const tempPath = path.join(tempFolderPath, `imports-${tempPathId}.ts`)

  const tempFilesImportWithinObject = globFiles.map((globFile, i) => {
    const key = globFromNormalizedBase[i]
    const value = normalizePathToPosix(path.relative(tempFolderPath, globFile))

    const part = `'${key}': await import('${value}')`
  ...
  fs.writeFileSync(
    tempPath,
    createTextForGlobResolvedImportFile({
      cwd,
      absPath,
      specifier,
      imports: tempFilesImportWithinObject
    })
  )
  
  return url.pathToFileURL(tempPath)
}
